...
local function open_dim()
  if not config.background_dim_on_explain then
    return
  end
...
local function focused_box_right_col()
  ...
  if not line or #line.stations == 0 then
    return LABEL_W + 1 + 2
  end
...
function M.build_info_rows(title, summary, file_id, detail, connections)
  ...
  if detail and not detail.error then
    ...
    for _, wl in ipairs(util.word_wrap(summary_text, W - 4)) do
      push("  " .. wl)
    end
    blank()
    ...
    if detail.file then
      push(
        "  " .. detail.file .. "  :" .. (detail.line_start or "") .. "–" .. (detail.line_end or ""),
    ...
    if detail.calls and #detail.calls > 0 then
      ...
      for _, c in ipairs(detail.calls) do
        ...
        push(name .. (hint ~= "" and "  " .. hint or ""), { file = c.file, line = c.line_start })
      end
      blank()
    end
    ...
    if detail.called_by and #detail.called_by > 0 then
      ...
      for _, c in ipairs(detail.called_by) do
        ...
        push(name .. (loc ~= "" and "  " .. loc or ""), { file = c.file, line = c.line_start })
      end
      blank()
    end
  else
    ...
    for _, wl in ipairs(util.word_wrap(s, W - 4)) do
      push("  " .. wl)
    end
    blank()
  end

  if connections and ((#(connections.calls_into or {}) > 0) or (#(connections.called_from or {}) > 0)) then
...
function M.build_module_info_rows(m)
  ...
  for _, wl in ipairs(util.word_wrap(summary, W - 4)) do
    push("  " .. wl)
  end
  blank()
...
function M.open_info_popup(title, rows, W, jump_targets, on_close_cb, station_id)
  ...
  vim.keymap.set("n", "<CR>", function()
    ...
    if not target or not target.file or target.file == "" then
      return
    end
    M.close_info()
    if on_close_cb then
...
function M.open_explain_float(station_id, station_name)
  ...
  -- nil station_id = blank (cursor on empty/unindexed line)
  local explanation = ""
  if station_id then
    -- Same station already shown — nothing to do
    if M.explain_station_id == station_id then
      return
    end

    local detail = fetch(config.server .. "/station/" .. station_id)
    explanation = (detail and detail.explanation ~= "" and detail.explanation)
      or "(no explanation available — re-index to generate)"
  end
  ...
  local title = station_id and ("  " .. (station_name or station_id) .. "  ") or "  metroscope  "

  -- Reuse existing window if open — just update content
  if M.explain_win and vim.api.nvim_win_is_valid(M.explain_win) then
    local buf = vim.api.nvim_win_get_buf(M.explain_win)
    vim.bo[buf].modifiable = true
    vim.api.nvim_buf_set_lines(buf, 0, -1, false, rows)
    vim.bo[buf].modifiable = false
    vim.api.nvim_win_set_config(M.explain_win, { title = title, title_pos = "center", height = H })
