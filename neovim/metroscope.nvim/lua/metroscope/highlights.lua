...
function M.apply()
  if not state.buf or not state.data then
    return
  end
    ...
    for si, s in ipairs(line.stations) do
      ...
      if s.has_cross_line then
        ...
        while true do
          ...
          vim.api.nvim_buf_add_highlight(state.buf, ns, "MetroscopeCrossLine", base + 1, p - 1, p - 1 + #CROSS_SYM)
          pos = p + #CROSS_SYM
        end
      end
    end
